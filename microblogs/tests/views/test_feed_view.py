...
class FeedViewTestCase(TestCase):
    ...
    def test_get_get_feed_redirects_when_not_logged_in(self):
    ...
    def test_feed_shows_users_own_posts(self):
        ...
        response_url = self.client.get(self.url)
        for count in range (100,103):
            self.assertNotContains(response_url, f'Post__{count}')
        for count in range (200,203):
            self.assertContains(response_url, f'Post__{count}')
